...
const RESERVED_SECTORS = 4;             /* boot sector + 3 reserved (FreeDOS compat) */
const NUM_FATS = 2;
...
function calcGeometry(partitionSectors) {
    /* Try cluster sizes from 4 to 64 sectors (skip 1 and 2 for FreeDOS compat) */
    for (const spc of [4, 8, 16, 32, 64]) {
...
function createBlankImage(sizeMB = 32) {
    ...
    /* Partition type: 0x04 for FAT16 < 32MB, 0x06 for >= 32MB */
    img[pe + 4] = (partitionSectors < 65536) ? 0x04 : 0x06;

    /* CHS end: compute from partition end LBA */
    const endLBA = PARTITION_START_LBA + partitionSectors - 1;
    const endC = Math.floor(endLBA / (heads * secPerTrack));
    const endH = Math.floor((endLBA % (heads * secPerTrack)) / secPerTrack);
    const endS = (endLBA % secPerTrack) + 1;
    if (endC > 1023) {
        /* LBA mode marker for large disks */
        img[pe + 5] = 0xFE; img[pe + 6] = 0xFF; img[pe + 7] = 0xFF;
    } else {
        img[pe + 5] = endH & 0xFF;
        img[pe + 6] = (endS & 0x3F) | ((endC >> 2) & 0xC0);
        img[pe + 7] = endC & 0xFF;
    }
    ...
    /* Total sectors: use 16-bit field when value fits, else 32-bit */
    if (partitionSectors < 65536) {
        img.writeUInt16LE(partitionSectors, bpb + 19);
        img.writeUInt32LE(0, bpb + 32);
    } else {
