...
import { ROWS, COLS, CP437, TC_NORMAL, TC_ESC, TC_CSI, TC_MARKER } from './constants.js';
 ...
 */
export function initTextCapBuffer() {
    state.textCapBuffer = [];
    for (let r = 0; r < ROWS; r++) {
        state.textCapBuffer[r] = new Array(COLS).fill(0x20);
    }
    state.textCapCurRow = 0;
    state.textCapCurCol = 0;
    state.textCapDirty = false;
    state.textCapParseState = TC_NORMAL;
    state.textCapCsiParams = "";
}
 ...
 */
export function textCapRowToString(r) {
    if (!state.textCapBuffer || !state.textCapBuffer[r]) return " ".repeat(COLS);
    let s = "";
    for (let c = 0; c < COLS; c++) {
        const code = state.textCapBuffer[r][c];
 ...
 */
export function textCapParseByte(byte) {
    switch (state.textCapParseState) {

    case TC_NORMAL:
        if (byte === 0x1B) {
            /* ESC — begin escape sequence */
            state.textCapParseState = TC_ESC;
        } else if (byte === 0x0D) {
            ...
            state.textCapCurRow++;
            if (state.textCapCurRow >= ROWS) {
                textCapScrollUp();
                state.textCapCurRow = ROWS - 1;
            }
            state.textCapDirty = true;
        } else if (byte === 0x08) {
            /* Backspace — move cursor left */
            if (state.textCapCurCol > 0) state.textCapCurCol--;
        } else if (byte >= 0x20) {
            /* Printable character — write to buffer and advance */
            if (state.textCapCurRow >= 0 && state.textCapCurRow < ROWS &&
                state.textCapCurCol >= 0 && state.textCapCurCol < COLS) {
                state.textCapBuffer[state.textCapCurRow][state.textCapCurCol] = byte;
                state.textCapDirty = true;
            }
            state.textCapCurCol++;
            if (state.textCapCurCol >= COLS) {
                /* Wrap to next line */
                state.textCapCurCol = 0;
                state.textCapCurRow++;
                if (state.textCapCurRow >= ROWS) {
    ...
    case TC_ESC:
        if (byte === 0x5B) {
            ...
            state.textCapParseState = TC_CSI;
            state.textCapCsiParams = "";
        } else {
    ...
    case TC_CSI:
        if (byte >= 0x30 && byte <= 0x3F) {
            ...
            state.textCapCsiParams += String.fromCharCode(byte);
        } else if (byte >= 0x20 && byte <= 0x2F) {
            /* Intermediate byte — accumulate but we don't use these */
            state.textCapCsiParams += String.fromCharCode(byte);
        } else {
            /* Final byte — dispatch the CSI command */
            textCapDispatchCSI(byte, state.textCapCsiParams);
            state.textCapParseState = TC_NORMAL;
        }
        break;
 ...
 */
export function textCapDispatchCSI(cmd, params) {
    ...
    case 0x48: /* 'H' — Cursor Position: ESC[row;colH (defaults 1;1) */
        state.textCapCurRow = Math.max(0, Math.min(ROWS - 1, (parts[0] > 0 ? parts[0] : 1) - 1));
        state.textCapCurCol = Math.max(0, Math.min(COLS - 1, (parts[1] > 0 ? parts[1] : 1) - 1));

        /* Auto-clear from cursor to end of line when the game repositions. */
        for (let c = state.textCapCurCol; c < COLS; c++) {
            state.textCapBuffer[state.textCapCurRow][c] = 0x20;
        }
        state.textCapDirty = true;
        break;
    ...
    case 0x4A: /* 'J' — Erase in Display (default 0, we handle 2) */
        if (parts[0] === 2) {
            /* Clear entire screen */
            for (let r = 0; r < ROWS; r++) {
                state.textCapBuffer[r].fill(0x20);
            }
            state.textCapDirty = true;
        }
        break;
 ...
 */
export function textCapScrollUp() {
    if (!state.textCapBuffer) return;
    state.textCapBuffer.shift();
    state.textCapBuffer.push(new Array(COLS).fill(0x20));
    state.textCapDirty = true;
}
 ...
 */
export function renderTextCapScreen() {
    if (!state.textCapBuffer || !state.textCapDirty) return;
    state.textCapDirty = false;
 ...
 */
export function checkTextCapMarker(byte) {
    if (byte === TC_MARKER[state.textCapMarkerPos]) {
        state.textCapMarkerPos++;
        if (state.textCapMarkerPos >= TC_MARKER.length) {
            state.textCapMarkerPos = 0;
